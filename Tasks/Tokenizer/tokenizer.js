...
function replaceProps(obj, parent, includeSet, excludeSet) {
    for (var _i = 0, _a = Object.getOwnPropertyNames(obj); _i < _a.length; _i++) {
        var prop = _a[_i];
        var propPath = parent === '' ? "" + prop : parent + "." + prop;
        if (obj[prop] instanceof Array) {
            obj[prop].forEach(function (arrayObj, position) {
                propPath += "[" + position + "]";
...
function run() {
    try {
        ...
        var tokenizeType = tl.getInput("tokenizeType", true);
        var includes = tl.getInput("includes", false);
        var excludes = tl.getInput("excludes", false);
        if (!includes) {
            includes = '';
        }
        if (!excludes) {
